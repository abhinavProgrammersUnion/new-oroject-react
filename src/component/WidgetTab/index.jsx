...
const WidgetTabPane = ({ arr, a_id, id, dark, category }) => {

  const news = useContext(MyContext)?.newsData?.data

  const fetchDataByCategory = news?.filter((data, index) => data.category == category)

  console.log(fetchDataByCategory);

  return (
    <Fade in={id === a_id}>
      <div className="widget tab_widgets">
        {fetchDataByCategory?.map((item, i) => (
          <div key={i}>
            <div className="single_post widgets_small">
              ...
              <div className="single_post_text">
                <div className="meta2 meta_separator1">
                  <Link to={`/${item.category}/${item.title.split(' ').join('-')}`}>{item?.category}</Link>
                  <Link to={`/${item.category}/${item.title.split(' ').join('-')}`}>{item?.publish_date}</Link>
                </div>
                <h4>
                  <Link to={`/${item.category}/${item.title.split(' ').join('-')}`}>{item.title}</Link>
                </h4>
              </div>
            </div>
...
const WidgetTab = ({ className, dark }) => {
  ...
  news?.map((data, index) => { categoryArray.push(data.category) })
  ...
  return (
    <div className={`widget_tab md-mt-30 ${className}`}>
      ...
      <TabContent activeTab={activeTab}>
        <TabPane tabId="1">
          <WidgetTabPane dark={dark} a_id={activeTab} id="1" category="Travel" arr={data} />
        </TabPane>
        <TabPane tabId="2">
          <WidgetTabPane dark={dark} a_id={activeTab} id="2" category="world" arr={data} />
        </TabPane>
        <TabPane tabId="3">
          <WidgetTabPane dark={dark} a_id={activeTab} id="3" category="science" arr={data} />
        </TabPane>
      </TabContent>
    </div>
  );
